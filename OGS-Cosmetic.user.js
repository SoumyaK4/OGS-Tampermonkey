...
(function () {
    ...
    /** Add OGS Logo to top left */
    const addTopLeftLogo = () => {
        if (document.getElementById('ogs-top-left-logo')) return;
        const logo = document.createElement('a');
        logo.id = 'ogs-top-left-logo';
        logo.href = 'https://online-go.com/';
        logo.style.cssText = `
            position: fixed;
            top: 1px;
            left: 45px;
        ...
        `;
        logo.onmouseover = () => logo.style.opacity = '1';
        logo.onmouseout = () => logo.style.opacity = '0.5';
        logo.innerHTML = `<img src="https://raw.githubusercontent.com/online-go/online-go.com/main/assets/ogs_bw.svg" style="width: 80px; height: 80px; filter: drop-shadow(0 0 2px rgba(0,0,0,0.3));">`;
    ...
    const injectCSS = () => {
        const css = `
          :root { --dock-width: 11rem !important; }
    ...
    margin-right: 4px !important;
          ...
          .Dock:hover {
            opacity: 1;
                    ...
                    .Dock .TooltipContainer {
            margin: 4px 0;
            font-size: 14px; /* uniform font size */
          }
          .Dock .TooltipContainer i,
          .Dock .TooltipContainer img {
            width: 20px;
            height: 20px;
            font-size: 14px;
          }
    ...
    `;
        ...
        let enabled = false;
        let timingScript = null;
        mtButton.addEventListener('click', () => {
            if (!enabled) {
                timingScript = document.createElement("script");
                timingScript.src = `https://psalaets.github.io/ogs-move-timing/bookmarklet.js?${Date.now()}`;
                timingScript.type = "module";
                timingScript.onload = () => {
                    // script loaded, keep element for later removal
                };
    ...
    const removeDockItems = async () => {
        ...
        const removeTexts = [
            'Tournament', 'Zen mode', 'Analyze game', 'Toggle coordinates', 'Link to game', 'Add to library', 'Download SGF', 'SGF with comments', 'Link to review'
        ];
        items.forEach(item => {
